...
| --- | --- |
| 空间代数微基准 | 表现混合；多数操作快 `1.22x` 到 `3.16x`，`cross_force` 仍然较慢 |
| 串联链 RNEA | Kinetix 快 `1.32x` 到 `1.61x` |
| 串联链 ABA | Kinetix 快 `1.49x` 到 `1.77x` |
| 串联链 CRBA | 2/6 自由度略慢，从 7 自由度开始快 `1.05x` 到 `1.28x` |
...
这份结果已经包含第一阶段 CRBA 热路径优化：通用 6x6 空间惯量/力变换被替换为直接 3D 分块运算，
并加入了 Z 轴关节投影特化。接下来更值得继续压的是小自由度下的常量开销，以及剩余 3x3 临时矩阵流量。
